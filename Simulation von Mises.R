...
GroupedR<-function(n,k,Mu, kappa,p){
...
GroupedRSimu<-function(n,k,Mu, kappa, p){
  ...
  for(i in 1:Simu){
    ...
    Estimations[i,]<-GroupedR(n=n,k=k,Mu=pi, kappa=kappa,p)$Results
  }
...
for (i in 1:nrow(param_combinations)) {
  ...
  simulation_results[i,] <- GroupedRSimu(n = n, k = k, Mu = pi, kappa = kappa,p)$Indicators
}

results_df <- cbind(param_combinations, simulation_results)
colnames(results_df) <- c("n","k","kappa","TrueMean","bias_uncorrected","bias_cofR2","bias_cofRhPi","mse_uncorrected","mse_cofR2","mse_cofRhPi")


# Write the results to an Excel file
write.xlsx(results_df, "SimulationVonMises=1.xlsx")
